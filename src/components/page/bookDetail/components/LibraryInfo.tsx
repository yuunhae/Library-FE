      ...
      <section>
        ...
        >
          <input
            ...
            className="w-[93%] h-full border  border-border-color px-3 text-sm placeholder-gray-400"
          />
          <button
            onClick={() => setIsNationWide((prev) => !prev)}
            className={`w-[7%]  h-full text-sm border border-solid border-border-color hover:bg-point-color  hover:text-white transition-colors ${isNationwide ? "bg-point-color text-white" : "bg-white"}`}
          >
            전국
          </button>
        </form>
